...
namespace noa {
	...
	inline NOA_API unsigned char LOBYTE(unsigned int w) {
		return static_cast<unsigned char>(w & 0xff);
	}

	inline NOA_API unsigned int BGR(unsigned int color) {
		return (((color) & 0xFF) << 16) | ((color) & 0xFF00FF00) | (((color) & 0xFF0000) >> 16);
	}

	inline NOA_API unsigned int RGB(unsigned char r, unsigned char g, unsigned char b) {
	...
	inline NOA_API unsigned char GetRValue(unsigned int rgb) {
		return LOBYTE(rgb);
	}

	inline NOA_API unsigned char GetGValue(unsigned int rgb) {
		return LOBYTE(static_cast<unsigned short>(rgb) >> 8);
	}

	inline NOA_API unsigned char GetBValue(unsigned int rgb) {
		return LOBYTE(rgb >> 16);
	}

	inline NOA_API unsigned int RGBA(unsigned char r, unsigned char g, unsigned char b, unsigned char a) {
	...
	inline NOA_API unsigned char GetAValue(unsigned int argb) {
		return LOBYTE(argb >> 24);
	}

	inline NOA_API unsigned int MULTICOLOR(unsigned int color, unsigned int mutiColor) {
	...
	enum NOA_API Color : unsigned int {
		BLACK			= 0xFF000000,
		BLUE			= 0xFFAA0000,
		GREEN			= 0xFF00AA00,
		CYAN			= 0xFFAAAA00,
