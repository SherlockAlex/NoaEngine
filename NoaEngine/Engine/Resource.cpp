...
noa::SpriteFile noa::Resource::LoadSprite(const std::string& filename)
{
	...
	{
		Debug::Warring("读取失败：" + std::string(filename));
	}
...
noa::AnimationFile noa::Resource::LoadAnimationClip(const std::string& filename)
{
	...
	if (!inputFile) {
		Debug::Error("Opening file: " + std::string(filename) + " error");
		exit(-1);
	}
...
noa::Tile::Tile(std::shared_ptr<Sprite> sprite, bool isCollision)
{
	this->sprite = sprite;
...
{
	this->sprite = std::make_shared<Sprite>(spriteFile, Vector<int>(1, 1));
	this->isCollision = isCollision;
}
